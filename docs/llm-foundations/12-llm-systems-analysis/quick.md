...
Q 有 32 个头（投影输出宽 4096），共享后 K/V 各只有 8 个头（输出宽 1024）。头少 → 投影矩阵窄 → 参数少。这正是第 11 章"档案柜"在参数账本上的体现。
:::
...
**用哪种通信，由"下一道工序需要什么形状的数据"决定**：下一层要完整数据 → AllGather 拼回来；要按份求和 → AllReduce / ReduceScatter；要把 Token 按去向重分发 → AlltoAll。这不是背出来的，是**顺着张量切分方向推出来的**——第 1 章那句话在收官处闭环：**切法决定对账方式**。
...
| 单元 | 讲什么（白话） | 什么时候需要它 |
| --- | --- | --- |
| [12-1 读配置并估算参数量](./01-config-parameters.md) | 从图纸到清单的完整算例 | 想亲手拆一个模型配置 |
| [12-2 权重、KV Cache 与训练显存](./02-model-memory.md) | 仓库账的精算 | 想做部署容量规划 |
| [12-3 FLOPs 与完整数据流](./03-flops-dataflow.md) | 电费账 + 全链路数据流 | 想估训练/推理时间 |
| [12-4 模型切分、通信与性能模型](./04-sharding-performance.md) | 分店方案与通信推导 | 想设计并行策略 |
| [本章总结](./summary.md) | 阶段总结 + 自测 | 检验大模型基础整阶段 |

---

[返回第 12 章 →](../12-llm-systems-analysis.md) · [返回大模型基础目录 →](../index.md)
