# ⚡ 速通｜第 10 章：MoE 与 Expert Parallel（约 5 分钟）
...
和第 1 章的 AllReduce 对比一下语义：AllReduce 汇总的是**同一份东西的不同意见**（DP 的梯度）；AlltoAll 交换的是**不同的东西给不同的人**（MoE 的 Token）。
...
| 单元 | 讲什么（白话） | 什么时候需要它 |
| --- | --- | --- |
| [10-1 MoE、Router 与 Top-k](./01-moe-router-topk.md) | 分诊台的工作机制 | 想读懂 MoE 模型配置 |
| [10-2 负载均衡、Capacity 与丢 Token](./02-load-balance-capacity.md) | 挂号不均的连锁灾难 | 想分析 MoE 训练问题 |
| [10-3 Token 重排与 Alltoall](./03-token-alltoall.md) | 跨院区快递的完整细节 | 想理解 EP 通信 |
| [10-4 MoE 的并行组合与性能瓶颈](./04-moe-parallel-performance.md) | MoE + 其他并行的组合拳 | 想做 MoE 性能分析 |
| [本章总结](./summary.md) | 动手练习 + 自测 | 想检验整章掌握程度 |

---

[返回第 10 章 →](../10-moe-expert-parallel.md) · [返回大模型基础目录 →](../index.md)
