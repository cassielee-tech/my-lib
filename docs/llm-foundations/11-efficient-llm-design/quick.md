...
| 单元 | 讲什么（白话） | 什么时候需要它 |
| --- | --- | --- |
| [11-1 MHA、MQA 与 GQA](./01-mha-mqa-gqa.md) | 档案柜共享的完整机制 | 想算 KV Cache 收益 |
| [11-2 量化原理、粒度与显存收益](./02-quantization.md) | 口袋书工程的全部细节 | 想做量化部署 |
| [11-3 长上下文的四条优化路线](./03-long-context.md) | 四件工具的深入对比 | 想做长文本方案选型 |
| [11-4 把模型优化映射到算子与通信](./04-optimization-infra.md) | 从模型层下到系统层 | 想衔接 AI Infra 视角 |
| [本章总结](./summary.md) | 动手练习 + 自测 | 想检验整章掌握程度 |

---

[返回第 11 章 →](../11-efficient-llm-design.md) · [返回大模型基础目录 →](../index.md)
