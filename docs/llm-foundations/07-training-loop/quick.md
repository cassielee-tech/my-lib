...
| 单元 | 讲什么（白话） | 什么时候需要它 |
| --- | --- | --- |
| [07-1 一次训练 Step 与梯度](./01-training-step-gradient.md) | 学习闭环的完整拆解 | 想亲手跑通一个训练循环 |
| [07-2 梯度累积与 AdamW 状态](./02-gradient-accumulation-adamw.md) | 攒作业与两本账的细节 | 想算等效 batch / 理解优化器显存 |
| [07-3 学习率、混合精度与梯度裁剪](./03-lr-mixed-precision-clipping.md) | 三件学习技巧的原理与顺序 | 遇到精度/稳定性问题时 |
| [07-4 训练显存与梯度通信](./04-training-memory-communication.md) | 书桌都摆了什么、对账时机 | 想分析训练显存和重叠 |
| [本章总结](./summary.md) | 动手练习 + 自测 | 想检验整章掌握程度 |

---

[返回第 7 章 →](../07-training-loop.md) · [返回大模型基础目录 →](../index.md)
