...
```text
三本账（I05-1）：FLOPs · Bytes(min/实测) · AI = 两者之比
        ↓
一条屋脊（I05-2）：P = min(峰值算力, AI × 峰值带宽)
   左坡 = Memory-bound   屋顶 = Compute-bound
   拐点 = 峰值算力 ÷ 峰值带宽
        ↓
四个案例（I05-3）：GEMM 屋顶 · 逐元素深谷 · Norm 看实现 · FlashAttention 向右挪
        ↓
两个方向（I05-4）：向上爬（对齐/流水/tile）· 向右挪（融合/复用/精度/算法）
        ↓
第 2 阶段：整网执行——Eager、计算图与图编译
```
...
下一课进入第二阶段：**Eager、计算图与图编译**——从单算子视角上升到整网视角，看 Python 的一行代码怎样变成可执行计算图。
