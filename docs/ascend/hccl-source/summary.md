...
> 所属课程：[HCCL 源码学习](../hccl-source.md)

十个单元走完（概念地图 + 逐段精读双轨），把所有结论压缩成一张可复用的地图，并给出继续深入的路线。
...
扩展            MC2 自定义算子：Kfc 参数包 / 资源 API 两条路径（单元 9）
```
...
| 单元 | 主题 | 概念来源 | 精读对象 |
| --- | --- | --- | --- |
| 0 | HCCL 全景与仓库地图 | 官方架构简介 | 仓库目录、调用链总览 |
| 1 | 软件架构：分层与对外 API | 官方架构简介 | `include/hccl.h`（262 行） |
| 2 | 通信域、Rank 与 RankGraph | 官方架构简介 | —（hcomm 仓，见 HCOMM 课程） |
| 3 | 通信原语与同步机制 | 官方架构简介 | —（hcomm 仓，见 HCOMM 课程） |
| 4 | 通信引擎与任务执行 | 官方架构简介 2.4 节 | `comm_engine_utils.h` + 三引擎对照 |
| 5 | 集合算法与 selector 选择器 | 官方算法文档 | `op_common/selector/` + `all_reduce/selector/`（728 行） |
| 6 | AllReduce 调用链走读 | 官方用户指南 | `all_reduce_op.cc`（281 行） |
| 7 | executor 与 template | — | `op_common/executor/` + `template/`（13 个执行器） |
| 8 | 资源地基与 dlsym 解耦 | — | `op_common.cc` 资源链 + `topo_host.cc`（1132 行）+ `hcomm_dlsym/`（29 个文件） |
| 9 | MC2 自定义算子框架 | — | `hccl_mc2.h` + `examples/04、05` |

## 必须带走的概念清单

| 概念 | 一句话 | 出处 |
| --- | --- | --- |
| 三层软件结构 | HCCL 算子层 → HCOMM 域管理层 → HCOMM 基础通信层 | 单元 1 |
| 五层 API | L1 算子 / L2 域 / L2 拓扑资源 / L3 原语 / L3 资源 | 单元 1 |
| 控制面/数据面 | 建图管资源 vs Write/Read/Notify 搬数据 | 单元 1、3 |
| RankGraph | Node/Endpoint/Edge/Link/netLayer/Fabric/TopoInstance 建模"谁和谁怎么连" | 单元 2 |
| 递进链 | Edge（谁连）→ Link（怎么建）→ Channel（怎么通信） | 单元 2 |
| 两种语义 | 网络语义（RoCE/UB）vs 内存语义（HCCS/UB_MEM） | 单元 3 |
| 两种同步 | ThreadNotify（实体内）/ ChannelNotify（跨实体） | 单元 3 |
| 四种引擎 | AICPU_TS / CPU_TS / AIV / CCU，"Thread + 调度器 + 硬件"统一模型 | 单元 4 |
| 引擎瀑布 | 专用→通用的降级链，NOT_MATCH 是正规信号 | 单元 4、5 |
| α-β-γ 模型 | 步数 × α + 数据量 × β(+γ)，算法比较的钥匙 | 单元 5 |
| 分级通信 | 大数据量放机内高带宽域，机间传输压到最少 | 单元 5 |
| 算子三段式 | op.cc 入口 → selector 策略 → template/executor 机制 | 单元 6 |
| 快速路径 | 同 shape 高频重复 → 用缓存换规划开销 | 单元 6 |
...
| MC2 两条路径 | Kfc 参数包声明式 vs 资源 API + 原语手写 | 单元 9 |

贯穿全程的四个设计模式（单元 9 展开）：**静态注册+工厂**（两个注册表）、**责任链降级**（引擎瀑布/回退记忆/弱符号桩）、**序列化缓存**（engineCtx 全家桶）、**字符串解耦**（算法名/tag 连接一切）。
...
- 跑通 `examples/04_custom_ops_p2p`、`05_custom_ops_allgather`（单元 9 已走读）；
- 这是 HCOMM 新开放算子编程接口（`hccl_res.h`/`hccl_rank_graph.h` + `hcomm_primitives.h`）的官方落地路径——**直接衔接姊妹课程 [HCOMM 源码学习](../hcomm-source.md)**。
...
- [ ] 我能用 α-β-γ 模型解释 Ring 与 RHD 的适用场景，并读懂 selector 的阈值代码
- [ ] 我能复述 AllReduce 从建域到销毁的完整链路，并定位每段在源码的位置（单元 6 的调用链全景图）
- [ ] 我能解释算法名的组合公式，说出 Mesh RS 的真实语义
- [ ] 我知道 topoInfo、执行计划、回退记忆分别缓存在哪、key 是什么
...
读完本课程，继续进入姊妹篇 **[HCOMM 源码学习](../hcomm-source.md)**——下两层深潜与自定义通信算子开发。

---

[返回课程导学 →](../hccl-source.md) · [返回专栏目录 →](../index.md)
