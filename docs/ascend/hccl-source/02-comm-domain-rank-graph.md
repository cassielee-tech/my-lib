# 单元 2｜通信域、Rank 与 RankGraph

> 所属课程：[HCCL 源码学习](../hccl-source.md) · 第 2 单元（共 10 单元）
...
::: tip 本单元的精读对象在 hcomm 仓
RankGraph 的建模与实现代码位于 [hcomm 仓库](https://gitcode.com/cann/hcomm) 的 `coll_communicator_mgr/rank_graph/`，不在 hccl 仓——因此本单元以概念地图为主；hccl 仓侧消费拓扑的 `op_common/topo/` 在单元 8 精读，hcomm 仓的源码走读见姊妹课程 **[HCOMM 源码学习 · 2｜控制面：通信域的一生](../hcomm-source/02-control-plane-communicator.md)**。
:::
...
- **代码落点**：hcomm 仓 `coll_communicator_mgr/rank_graph/`（走读见 [HCOMM 源码学习](../hcomm-source.md)）、hccl 仓 `src/ops/op_common/topo/`（单元 8 精读）。
...
下一单元进入 **[3｜通信原语与同步机制](03-primitives-and-sync.md)**。
