...
> 所属课程：[HCCL 源码学习](../hccl-source.md) · 第 3 单元（共 10 单元）
...
数据面原语的实现位于 [hcomm 仓库](https://gitcode.com/cann/hcomm) 的 `base_comm/primitives/` 与 `base_comm/resource/`，不在 hccl 仓——本单元以概念地图为主；hccl 仓侧对这些原语的调用（wrapper 层）在单元 7 精读，hcomm 仓数据面走读见 **[HCOMM 源码学习 · 3｜数据面：原语与资源](../hcomm-source/03-data-plane-primitives.md)**。
:::
...
- **读码落点**：hcomm 仓 `base_comm/primitives/`（原语）与 `base_comm/resource/`（资源）；hccl 仓 executor/template 里编排的就是这些动词（单元 7）。
...
下一单元进入 **[4｜通信引擎与任务执行](04-comm-engines.md)**。
