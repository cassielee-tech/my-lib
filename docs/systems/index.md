...
| 5 | 推理引擎与 KV Cache 管理【选修】 | Continuous Batching、Paged Attention 和 Prefix Cache 怎样提高并发？ | 🚧 待写 |
| 6 | Prefill/Decode 调度与推理指标【选修】 | TTFT、TPOT、吞吐为什么互相制约？PD 分离解决什么？ | 🚧 待写 |

## 阅读提示

- **主线只需第 1、2 章**：先看模型侧的训练循环（第 1 章），再用系统侧的显存账本（第 2 章）落地；
- 第 3-6 章为推理/后训练视角：HCCL 岗位的推理相关价值集中在推理通信（PD 分离、多卡推理），可后置；
