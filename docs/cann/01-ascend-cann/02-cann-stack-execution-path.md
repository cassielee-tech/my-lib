...
从全栈视角看，昇腾平台从上到下分为：**应用层**（大模型、推荐、CV）→ **框架层**（PyTorch、MindSpore）→ **CANN** → **硬件层**（昇腾 AI 处理器）。CANN 是**承上启下的枢纽**——这正是专栏首页那张全景图的出处：
...
当预置算子不够用时，就用 **Ascend C**（本专栏第 3、4 章的主角）开发自定义算子。
...
完整对照表见笔记 [CANN Learning Hub](../cann-learning-hub.md)。**概念相通，只是名字不同**——这也是专栏反复用 CUDA 概念类比的原因。
...
至此，第 6 章"从 PyTorch 走向 HCCL"的每个环节都在本单元露过面了。
