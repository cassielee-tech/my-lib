...
**跟不动？** 先读 [⚡ 速通版（约 5 分钟）](#⚡-速通-约-5-分钟)：一场文字接龙考试类比讲完整章，再回来按单元深入。
...
- **03-1（约 15 分钟）**：[Tokenization：文本变成整数序列](#_03-1-tokenization-文本变成整数序列)
- **03-2（约 15 分钟）**：[特殊 Token、Mask 与 Embedding](#_03-2-特殊-token、mask-与-embedding)
- **03-3（约 15 分钟）**：[训练标签与下一个 Token 预测](#_03-3-训练标签与下一个-token-预测)
- **03-4（约 15 分钟）**：[完整 Shape 流程与系统代价](#_03-4-完整-shape-流程与系统代价)
...
| 单元 | 讲什么（白话） | 什么时候需要它 |
| --- | --- | --- |
| [03-1 Tokenization](#_03-1-tokenization-文本变成整数序列) | 怎么剪、为什么这样剪 | 想理解中英文切分差异 |
| [03-2 特殊 Token、Mask 与 Embedding](#_03-2-特殊-token、mask-与-embedding) | 两块挡板 + 查表细节 | 想亲手处理一批输入 |
| [03-3 训练标签与下一个 Token 预测](#_03-3-训练标签与下一个-token-预测) | 接龙考试的完整出题法 | 想搞清 Loss 到底怎么算 |
| [03-4 完整 Shape 流程与系统代价](#_03-4-完整-shape-流程与系统代价) | 尺寸链 + 词表大小的系统代价 | 想估算显存和计算量 |
| [本章总结](#本章总结) | 动手任务 + 理解检查 | 想检验整章掌握程度 |

## 03-1｜Tokenization：文本变成整数序列
...
## 03-2｜特殊 Token、Mask 与 Embedding
